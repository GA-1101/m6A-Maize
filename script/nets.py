...
class WeakRMLSTM(tf.keras.Model):
    ...
    def __init__(self):
        ...
        self.conv1 = tf.keras.layers.Conv2D(32, (1, 15), padding='same', activation='relu')
        self.lstm = tfkl.TimeDistributed(
            tfkl.Bidirectional(
                tfkl.GRU(16, return_sequences=True, dropout=0.0, recurrent_dropout=0.0, kernel_regularizer=l2(0.005)),
                merge_mode='sum'
            )
        )
