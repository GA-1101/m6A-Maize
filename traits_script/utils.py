...
def one_hot_bag_embedding(sequence, instance_len, instance_stride):
...
def embed(sequence, instance_len, instance_stride, zero_pad=True):
    ...
    bag = []
    for i in np.arange(np.floor(instance_num)+(gap > 0)):
        instance = padded_seq[int(i*instance_stride):int(i*instance_stride+instance_len)]
        bag.append(instance)
    bag = np.stack(bag).astype(np.int32)
    one_hot_bag = np.eye(5)[bag - 1][:, :, :4].astype(np.float32)
