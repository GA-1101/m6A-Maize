...
<?php
//读取结果
//$resultsFile = fopen("/var/www/html/maize/script/test.txt", "r") or die("Unable to open file!");
$resultsFile = fopen("/var/www/html/maize/job/".$ID."/download/Results/output.txt", "r") or die("Unable to open file!");
//$resultsData = fgets($resultsFile);
$resultsArray = array();
while(!feof($resultsFile)) {
    
    $resultsStr = fgets($resultsFile); //fgets()函数从文件指针中读取一行
    array_push($resultsArray, $resultsStr);
    $i++;
}
$arrlength = count($resultsArray) - 1; //减去output.txt文件多的一空行
    ...
    <div class="container" style="margin-top: 90px;">
        ...
        <div class="thumbnail" style="padding-left:20px;padding-right:20px;padding-top:18px;color:black; border-left-color:#000557;border-left-width:5px">
            ...
            <main class="row">
                <div class="col">
                    <br>
                    <div class="category-annotation" style="background-color:#F5F5F5; padding-left:2px">
                        <label style="font-size:17px; margin-top:12px" for="">The existence index of m6A sites:</label>
                        <ul>
                            <li class="sites-category"><b>(No data):</b> The sequence failed to obtain any data.</li><br>
                            <li class="sites-category"><b>< 0.5:</b> There is no m6A site in this sequence.</li><br>
                            <li class="sites-category"><b>>= 0.5:</b> m6A site exists in this sequence.</li><br> 
                        </ul>
                    </div>
                    <br>
                    <br>
...
<?php
    for ($x=0; $x<$arrlength; $x++){
                echo"<tbody>";
                    echo"<tr>";
                        echo"<td style='color:#964B29'>".$ID."</td>";
                        echo"<td id='Status1' style='color:#964B29'>Completed</td>";
                        echo"<td><font style='color:#964B29'>".$resultsArray[$x]."</font></td>";
                        
                        if($resultsArray[$x] < 0.5){
                            echo "<td><i class='fa fa-times-circle' aria-hidden='true'></i></td>";
                        } else {
                            echo "<td><i class='fa fa-check-circle' aria-hidden='true'></i></td>";
                        }
